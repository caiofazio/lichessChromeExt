...
self.onmessage = function (e) {
    ...
    if (e.data.match("^position") == "position") {
        ...
        g_timeout = parseInt("500", 10);
        ////console.log(FinishMoveLocalTesting, 99, FinishPlyCallback)
...
const turnChangeFa = function(mutationsList, observer) {
    console.log(mySide)
    console.log(toMove)
    if(toMove != mySide){
        console.log('chamou')	
        setTimeout(getFromTo, 50)
        toMove = 8 - toMove;	
    }else{	
        console.log('Waiting oponent move')	
        toMove = 8 - toMove;
    }	       
};

var CheckMudancaRodada;
function resetCheckMudanca(){
    //CheckMudancaRodada = setInterval(verificaMudancaRodada, 100)
}
var CheckFirstMove;
function WaitWhiteFm(){
    CheckFirstMove = setInterval(verificaFirstMove, 50)
}
function verificaFirstMove(){
    var rawLastMove = document.getElementsByClassName('last-move');
    console.log('fm')
    if(rawLastMove.length > 1){
        clearInterval(CheckFirstMove)
        targetNode = document.getElementsByClassName('rmoves')[0];
        const observer = new MutationObserver(turnChangeFa);
...
function verificaMudancaRodada() {
    let clockCheck = document.getElementsByClassName('rclock')[1].classList.length
    //console.log(clockCheck)
    ////console.log(clockCheck)
    //let count = bottom[0].childElementCount;
    ////console.log('Searching')
    if(mySide == 0){
        if(clockCheck == 3 && toMove == 8){
            clearInterval(CheckMudancaRodada)

            toMove = 0
            //console.log('Black now')
            getFromTo()
        }else if(clockCheck == 2 && toMove == 0){
            toMove = 8
            //console.log('White now1')
            //getFromTo()
        }
    }else{

        if(clockCheck == 3 && toMove == 0){
            clearInterval(CheckMudancaRodada)

            toMove = 8
            //console.log('White to play')
            getFromTo()
        }else if(clockCheck == 2 && toMove == 8){
            toMove = 0
            //console.log('Black to play1')
            //getFromTo()
        }
    }
    /*if(bottom[0].childElementCount == 1 && toMove == 0 && mySide == 8){

        ////console.log("White now");
        toMove = 8;
    }else if(toMove == 8 && bottom[0].childElementCount == 0 && mySide == 0){
        ////console.log("Black now");
        toMove = 0;
    }else if(mySide == 0 && bottom[0].childElementCount == 1 && toMove == 8){
        toMove = 0;
        ////console.log('Black now');
    }else if(mySide == 0 ){
        ////console.log('White now')
...
function invertWhiteOutPut(){
    outputTo[1] = 9 - outputTo[1]
    outputFrom[1] = 9 - outputFrom[1]
}
function invertBlackOutPut(){
    outputTo[0] = 9 - outputTo[0]
    outputFrom[0] = 9 - outputFrom[0]
}

function getMoveStringFa(){
    finalString = ""
    finalString += String.fromCharCode(96 + from[0])
    finalString += from[1]
    finalString += String.fromCharCode(96 + to[0])
    finalString += to[1]
    if(setupFirst){
    ////console.log('making first move ' + finalString )
    lastMoveFa = finalString
    MakeMove(GetMoveFromString(finalString))
    setupFirst = false;
    resetCheckMudanca()
    }else{
        ////console.log('aq')
    if(finalString == 'e1h1'){
        finalString = 'e1g1'
    }else if(finalString == 'e8a8'){
        finalString = 'e8c8'
    }else if(finalString == 'e8h8'){
        finalString = 'e8g8'
    }else if(finalString == 'e1a1'){
        finalString = 'e1c1'
    }
    console.log('oichamou')
    inputEngine()
...
function inputEngine(){
    console.log('FS :' +finalString + '== LMFA :' + lastMoveFa)
    if(finalString != lastMoveFa){
    console.log(finalString)
    lastMoveFa = finalString;
    let movefa = GetMoveFromString(finalString)
    MakeMove(movefa)
    Search(engineOutput, 40, null)
    }else{
        toMove = 8 - toMove
        resetCheckMudanca()
    }
}
function engineOutput(a,b,c){
    lastMoveFa = FormatMove(a)
    let svgaux = document.querySelector('svg')
    if(!fistSvgFa){
        if(svgaux.childElementCount > 2){
    svgaux.removeChild(svgaux.lastChild)
    svgaux.removeChild(svgaux.lastChild)
        }
    }else{
        fistSvgFa = false;
    }
    outputFrom[0] = lastMoveFa.charCodeAt(0) - 97 + 1
    outputFrom[1] = parseInt(lastMoveFa.charAt(1), 10)
    outputTo[0] = lastMoveFa.charCodeAt(2) - 97 + 1
    outputTo[1] = parseInt(lastMoveFa.charAt(3), 10)
    if(mySide == 8){
        invertWhiteOutPut()
    }else{
        invertBlackOutPut()
    ...
    let circle = document.createElementNS(svgns, 'circle');
        circle.setAttributeNS(null, 'cx', (piecesizefa * outputFrom[0]) - (piecesizefa/2) - 1);
        circle.setAttributeNS(null, 'cy', (piecesizefa * outputFrom[1]) - (piecesizefa/2) - 1);
        circle.setAttributeNS(null, 'r', piecesizefa/2);
        circle.setAttributeNS(null, 'style', 'fill: none; stroke: red; stroke-width: 5px; opacity: 1' );
        document.querySelector('svg').appendChild(circle);
    let circle1 = document.createElementNS(svgns, 'circle');
        circle1.setAttributeNS(null, 'cx', (piecesizefa * outputTo[0]) - (piecesizefa/2) - 1);
        circle1.setAttributeNS(null, 'cy', (piecesizefa * outputTo[1]) - (piecesizefa/2) - 1);
        circle1.setAttributeNS(null, 'r', piecesizefa/2);
        circle1.setAttributeNS(null, 'style', 'fill: none; stroke: blue; stroke-width: 5px; opacity: 1' );
        svgaux.appendChild(circle1)

    console.log(lastMoveFa)
    moveSquareFa.innerHTML = lastMoveFa
    resetCheckMudanca()
}

function getFromTo(){
//console.log
var boardlichess = document.getElementsByClassName('cg-wrap manipulable')
let width = boardlichess[0].offsetWidth;
piecesizefa = width * 0.125;
////console.log(piecesize)
var rawLastMove = document.getElementsByClassName('last-move')
var cookedLastMove = rawLastMove[0].style.transform.split(',')
cookedLastMove[0] = cookedLastMove[0].split('(')[1]
to[0] = Math.round((parseInt(cookedLastMove[0], 10)/piecesizefa) + 1)
to[1] = Math.round((parseInt(cookedLastMove[1], 10)/piecesizefa) + 1)
cookedLastMove = rawLastMove[1].style.transform.split(',')
cookedLastMove[0] = cookedLastMove[0].split('(')[1]
from[0] = Math.round((parseInt(cookedLastMove[0], 10)/piecesizefa) + 1)
from[1] = Math.round((parseInt(cookedLastMove[1], 10)/piecesizefa) + 1)
if(mySide == 8){
    invertWhite()
}else{
    setupFirst = false
...
function choiceHighlight(radio){
    ...
    for (var i = 0, len = group.length; i < len; i++){
        if(group[i].checked){
                group[i].nextSibling.style.border = 'solid blue'
                group[i].nextSibling.style.borderRadius = '50%'
                group[i].nextSibling.style.padding = '3px'

        }else{
            group[i].nextSibling.style.borderWidth = '0px'
            group[i].nextSibling.style.padding = '0px'
            group[i].nextSibling.style.paddingTop = '6px'
        }

}
}
    function inicializarFa(){
...
MainDiv.style.border = 'solid blue'
MainDiv.style.borderRadius = '10%'
MainDiv.style.display = 'flex'
MainDiv.style.flexDirection = 'column'
MainDiv.style.paddingTop = '20px'
MainDiv.style.justifyContent = 'start'
MainDiv.style.alignItems = 'center'
var TeamDiv = document.createElement('form')
MainDiv.appendChild(TeamDiv)
TeamDiv.setAttribute('class', 'teamForm')
TeamDiv.style.display = 'flex'
TeamDiv.style.flexDirection = 'row'
TeamDiv.style.width = '100%'
TeamDiv.style.justifyContent = 'start'
TeamDiv.style.margin = '20px'

var whiteSelect = document.createElement("INPUT");
whiteSelect.setAttribute("type", "radio");
whiteSelect.setAttribute("name", "side");
whiteSelect.setAttribute("value", "white");
//whiteSelect.addEventListener('change', choiceHighlight)
whiteSelect.style.margin = '0px 30px'
whiteSelect.style.opacity = '0'
var blackSelect = document.createElement("INPUT");
blackSelect.setAttribute("type", "radio");
blackSelect.setAttribute("name", "side");
blackSelect.setAttribute("value", "black");
//blackSelect.addEventListener('change', choiceHighlight)
blackSelect.style.margin = '0px 30px'
blackSelect.style.opacity = '0'

whiteSelect.id = 'whiteSelection'
blackSelect.id = 'blackSelection'
var blackSelectButton = document.createElement('label')
blackSelectButton.style.backgroundColor = 'black'
blackSelectButton.style.borderRadius = '50%'
blackSelectButton.style.width = '40px'
blackSelectButton.style.height = '40px'
blackSelectButton.setAttribute("for", "blackSelection")
blackSelectButton.style.fontSize = '20px'
blackSelectButton.style.fontWeight = 'bold'
blackSelectButton.innerHTML = 'B'
blackSelectButton.style.textAlign = 'center'
blackSelectButton.style.paddingTop = '6px'
blackSelectButton.style.color = 'gray'
var whiteSelectButton = document.createElement('label')
whiteSelectButton.style.backgroundColor = 'white'
whiteSelectButton.style.borderRadius = '50%'
whiteSelectButton.style.width = '40px'
whiteSelectButton.style.height =  '40px'
whiteSelectButton.setAttribute("for", "whiteSelection")
whiteSelectButton.style.fontSize = '20px'
whiteSelectButton.style.fontWeight = 'bold'
whiteSelectButton.innerHTML = 'W'
whiteSelectButton.style.textAlign = 'center'
whiteSelectButton.style.paddingTop = '6px'
whiteSelectButton.style.color = 'gray'

TeamDiv.appendChild(blackSelect)
TeamDiv.appendChild(blackSelectButton)
TeamDiv.appendChild(whiteSelect)
TeamDiv.appendChild(whiteSelectButton)

var StartButton = document.createElement('BUTTON')
MainDiv.appendChild(StartButton)
StartButton.addEventListener("click", inicializarFa);
...
moveSquareFa = document.createElement('p')
moveSquareFa.style.color = 'red'


MainDiv.appendChild(moveSquareFa)
